...
int Room::_roomNumberCounter = 100;
...
Room::Room(int beds = 0) {
    _roomNumber = ++_roomNumberCounter;
    _beds = beds;
    _guests = 0;
    _isAvailable = true;
}
...
bool Room::IsAvailableInInterval(const Date& from, const Date& to) const {
    return _isAvailable && (_accomodation > to || _departure < from);
}
